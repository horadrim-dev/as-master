...
class HeaderPluginPublisher(CMSPluginBase):
    ...
    def render(self, context, instance, placeholder):
        ...
        context['subtitle_position'] = instance.subtitle_position
        return context
...
@plugin_pool.register_plugin
class TabsPluginPublisher(GenerateIdMixin, CMSPluginBase):
    module = "Контейнеры"
    name = "Вкладки"
    model = TabsPlugin
    allow_children = True
...
@plugin_pool.register_plugin
class AccordionPluginPublisher(GenerateIdMixin, CMSPluginBase):
    module = "Контейнеры"
    name = "Аккордеон"
